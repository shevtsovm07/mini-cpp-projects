...
TEST(AggregateByKeyTest, AggregateByKeySingleKey) {
    std::vector<std::string> input = {"a", "a", "a"};
    auto result = AsDataFlow(input) 
    | AggregateByKey(0uz, [](const std::string&, size_t& count) {++count;}, [](const std::string& s){return s;})
    |AsVector();
    ASSERT_THAT(result, testing::ElementsAre(std::make_pair("a", 3)));
}